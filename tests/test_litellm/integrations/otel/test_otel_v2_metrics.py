...
def test_invalid_filter_logs_once_and_disables_metrics_without_raising():
    """A misconfigured filter must not break the request: the success hook does
    not raise, no metrics are recorded, and the misconfig is surfaced once (not
    per request) rather than silently swallowed at debug."""
    from unittest.mock import patch

    from litellm.integrations.otel import logger as v2_logger_module
    ...
    litellm.callback_settings = {
        "otel": {
            "attributes": {
                "include_list": ["gen_ai.request.model"],
    ...
    try:
        kwargs, response_obj, start, end = _build_call()
        with patch.object(v2_logger_module.verbose_logger, "error") as mock_error:
            asyncio.run(
                logger.async_log_success_event(kwargs, response_obj, start, end)
            )
            asyncio.run(
                logger.async_log_success_event(kwargs, response_obj, start, end)
            )
        assert mock_error.call_count == 1
        assert "metrics disabled" in mock_error.call_args.args[0]
    finally:
        litellm.callback_settings = previous

    assert not any(
        name.startswith("gen_ai.client.") for name in _metrics_by_name(reader)
    )
